...
- **Runtime `--palette` consistency:** The same report’s six PNGs must all be rendered with the **same** customer-confirmed `--palette` (`macaron` | `default` | `b` | `c`). Validator 1 requires `--palette`; use the same palette for validation and export. The slot file does not store palette, so if someone re-renders only `01_cover.png` with a different palette than cards 2–6, the **top header colors** will mismatch.
