...
1. **Create the output folder first** — determine where the final 5 PNGs will land (the folder passed as `--output-root/<stem>/`, or the default `output/<html_stem>/` inside the EP skill repo). Create it now if it does not exist.
...
This order guarantees the logo and the 5 PNGs are always co-located in the same folder, so nothing needs to be moved manually after export.
