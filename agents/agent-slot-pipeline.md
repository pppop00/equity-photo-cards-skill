...
End state: four PNGs (`01_cover.png`, `02_porter.png`, `03_five_year_financials.png`, `04_cfa_lens.png`) with **human-grade** Chinese copy **without** losing facts from the research HTML.
...
| A | Logo production | HTML identity + folder/slots logo check + web search if absent | `logo_official.png` in **output folder** (create it first) or existing valid folder logo; `card_slots.json` updated with **`logo_asset_path`** and **`cover_company_name_cn`** |
| B | Content production (Cards 1–3) | Report HTML + `financial_*.json` + `porter_analysis.json` + `analyst_call.json` + logo path | `card_slots.json` (Cards 1–3 draft) + `card_slots_worker_notes.json` |
| B.4 | CFA lens selector (Card 4) | Cards 1–3 draft + `analyst_call.json` + CFA progress hint (CLI / env / USER.md / default) | `cfa_lens` object merged into `card_slots.json` (see [cfa-lens-selector-agent.md](./cfa-lens-selector-agent.md)) |
| B.5 | Hardcode & logic audit | `card_slots.json` (draft) + normalized report facts | Same file, body copy verified (see [hardcode-audit-agent.md](./hardcode-audit-agent.md)) |
...
| — | Renderer | HTML + final `card_slots.json` | `output/<stem>/01_cover.png, 02_porter.png, 03_five_year_financials.png, 04_cfa_lens.png` |
...
- **Stage B** optimizes for *substance* on Cards 1–3: lift thesis, Porter evidence, 5-year arc, recent KPIs from the HTML; no pixel math.
- **Stage B.4 (CFA lens)** picks one CFA-syllabus concept that maps to this company's shape and writes the `cfa_lens` payload for Card 4. Runs after Cards 1–3 are drafted so the selector can see the business profile in context.
...
- **Stage C** optimizes for *constraints*: line wraps, character budgets, panel heights — without watering down meaning.
