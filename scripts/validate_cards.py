...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Cards 1-5 knowledge-map evidence gate. Each HTML must have a sibling "
            "<stem>.card_slots.json AND a sibling <stem>.card_slots_worker_notes.json "
            "with claim-level provenance for every material visible claim."
        )
    )
    ...
    for html in files:
        ...
        # Cards 1-5 claim-level evidence gate (blocking).
        ...
        if content_issues:
            overall_failed = True
            print(
                f"\n=== Card 1-5 claim-evidence gate FAILED for {html} ===",
                file=sys.stderr,
            )
            print(
                f"  slots:        {slots_path}\n"
            ...
            print(
                "  (Five-card knowledge-map evidence contract. See "
                "references/knowledge-map-v5.md.)",
                file=sys.stderr,
            )
            continue
